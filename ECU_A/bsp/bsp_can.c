	...
	if(CAN1->ESR & ((1 << 1) | (1 << 2)))  // EPVF(错误被动)或BOFF(总线关闭)
{
    ...
    else
    {
		...
		printf("[CAN] Send. TSR=0x%08X TME=%d%d%d\r\n", 
           CAN1->TSR,
           (CAN1->TSR>>26)&1, (CAN1->TSR>>27)&1, (CAN1->TSR>>28)&1);
    /*--------------------------------------------------
      清空邮箱控制寄存器
    --------------------------------------------------*/
    ...
    /*--------------------------------------------------
      ...
      TXRQ = 1
    --------------------------------------------------*/
		printf("MAILBOX=%d ID=%03X\r\n",
       mailbox,
       frame->id);

    CAN1->sTxMailBox[mailbox].TIR |= 1;

		printf("TIR=0x%08X TSR=0x%08X ESR=0x%08X\r\n",
