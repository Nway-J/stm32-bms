...
void XM25QH32_Write(uint8_t *buffer, uint32_t addr, uint16_t len)
{
    ...
    {
        /* 读取整个扇区 */
        XM25QH32_ReadData(sector_pos * SPI_FLASH_SECTOR_SIZE, SectorBuffer, SPI_FLASH_SECTOR_SIZE);
        ...
        if (i < sector_remain)
        {
            ...
            XM25QH32_SectorErase(sector_pos * SPI_FLASH_SECTOR_SIZE);

            /* 修改缓存 */

            for (i = 0; i < sector_remain; i++)
            {
                SectorBuffer[sector_offset + i] = buffer[i];
            }

            /* 写回整个扇区 */

            XM25QH32_Write_NoCheck(SectorBuffer, sector_pos * SPI_FLASH_SECTOR_SIZE,
        ...
        else
        {
            ...
            XM25QH32_Write_NoCheck(buffer, addr, sector_remain);
        }
