...
{
    f->id = CAN_ID_BMS_VOLTAGE; f->dlc = 8;
    f->data[0] = (mv >> 8) & 0xFF; f->data[1] = mv & 0xFF;
    f->data[2] = soc; f->data[3] = chg_state;
    for(int i=4;i<8;i++) f->data[i] = 0;
}

static inline void BMS_PackTemp(CAN_Frame_t *f, uint8_t temp)
{
    f->id = CAN_ID_BMS_TEMP; f->dlc = 8;
    f->data[0] = temp;
    for(int i=1;i<8;i++) f->data[i] = 0;
}

static inline void BMS_PackFault(CAN_Frame_t *f, uint8_t fault)
{
    f->id = CAN_ID_BMS_FAULT; f->dlc = 8;
    f->data[0] = fault;
    for(int i=1;i<8;i++) f->data[i] = 0;
}
