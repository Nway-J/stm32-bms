...
{
    if(v_cell >= 4.15f) return 100;
    if(v_cell <= 3.05f) return 0;
...
{
    uint8_t fault = 0;
    if(v_cell > 4.2f)          fault |= 0x01;  // 过压
    if(v_cell < 3.0f)          fault |= 0x02;  // 欠压
    if(temp_c  > 60.0f)        fault |= 0x04;  // 过温
    return fault;
}
...
static void Task_PowerOff(void)
{
    ...
    LED_ON(); for(volatile uint32_t i = 0; i < 500000; i++);
    LED_OFF(); for(volatile uint32_t i = 0; i < 500000; i++);
    LED_ON();  for(volatile uint32_t i = 0; i < 500000; i++);
    LED_OFF();
    // 直接设状态（没有事件源触发首次启动）
...
// 等待PB0按下启动
// 检测到按键后读取SOC：
//   SOC≤20% → Push EVENT_START（进入CHARGE）
//   SOC>20% → Push EVENT_START（进入DISCHARGE）
...
//        uint8_t soc_check = SOC_Simple(v_cell);

//        if(soc_check <= 20)
//        {
//            printf("[BMS] Low SOC (%d%%), CHARGE\r\n", soc_check);
//            // TODO: 需要区分EVENT_START进入CHARGE还是DISCHARGE
//            // 当前方案：BMS_EventHandle中EVENT_START固定进入DISCHARGE
//            // CHARGE由STANDBY中SOC≤20%时直接BMS_SetState进入
//            BMS_SetState(BMS_CHARGE);
//        }
//        else
//        {
//            printf("[BMS] Start\r\n");
...
static void Task_Discharge(void)
{
    ...
    // 1. 读取最新ADC值（双缓冲安全读取）
    if(!ADC_ReadSafe(&adc0, &adc1)) return;
	
//	 // ── CAN错误检测与恢复 ──
//    if(CAN1->ESR & ((1 << 1) | (1 << 2)))  // EPVF或BOFF
//    {
//        CAN1_Init();
//        printf("[BMS] CAN reinit\r\n");
//    }
    ...
    uint16_t mv = (uint16_t)(v_cell * 100);
    tx.id = 0x301; tx.dlc = 8;
    tx.data[0] = (mv >> 8) & 0xFF; 
		tx.data[1] = mv & 0xFF;
    tx.data[2] = soc;               
		tx.data[3] = CHG_STATE_DISCHARGE;
    for(int i=4;i<8;i++) tx.data[i] = 0;
//    CAN1_Send(&tx);
		uint8_t ret1 = CAN1_Send(&tx);
printf("301 ret=%d\r\n", ret1);
    ...
    // 5. 发送CAN 0x302（温度）
    tx.id = 0x302; tx.dlc = 8;
    tx.data[0] = (uint8_t)(temp_c + 0.5f);
    for(int i=1;i<8;i++) tx.data[i] = 0;
//    CAN1_Send(&tx);

		uint8_t ret2 = CAN1_Send(&tx);
printf("302 ret=%d\r\n", ret2);


    // 6. 日志输出
    printf("[BMS] DISCHARGE Cell=%.2fV SOC=%d%% T=%dC\r\n", v_cell, soc, (int)temp_c);
}
...
static void Task_Fault(void)
{
    ...
    if(now - last_blink > 200) { last_blink = now; LED_Toggle(); }
...
static void Task_Charge(void)
{
    ...
    if(charge_soc == 0) charge_soc = 50;

    // 模拟电量上升
    if(charge_soc < 95) charge_soc++;
    ...
    uint16_t mv = (uint16_t)(v_cell * 100);
    tx.id = 0x301; tx.dlc = 8;
    tx.data[0] = (mv >> 8) & 0xFF; tx.data[1] = mv & 0xFF;
    tx.data[2] = soc;               tx.data[3] = CHG_STATE_CHARGE;
    for(int i=4;i<8;i++) tx.data[i] = 0;
    CAN1_Send(&tx);
    ...
    printf("[BMS] CHARGING SOC=%d%% T=%dC\r\n", soc, (int)temp_c);

    // 充满自动停止
    if(charge_soc >= 95)
    {
        printf("[BMS] Charge complete!\r\n");
        charge_soc = 0; LED_OFF();
