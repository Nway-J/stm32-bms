...
硬件是单节电池电压与 NTC 温度监测平台。当前没有电流采样、均衡电路和充放电 MOS，因此 FAULT 只会触发软件报警、CAN 上报与日志记录，不能物理切断电流；SOC 是基于电压的简化显示估算，不是库仑计量。充放电 MOS 是计划中的后续升级，设计边界与控制链见[架构与数据流](docs/architecture.md#后续升级充放电-mos)。

## 先从这里开始

| 想了解什么 | 阅读入口 |
|---|---|
| 准备器件、接线、编译、烧录 | [复现教程](docs/getting-started.md) |
| 每个模块为何存在，谁读谁写 | [架构与数据流](docs/architecture.md) |
| 什么情况下报警，何时能够恢复 | [故障管理与注入实验](docs/faults-and-cli.md) |
| 三种 CAN 报文、计数器、CRC、超时 | [CAN 通信](docs/can.md) |
| 官方 FreeRTOS 文件到底改了什么 | [内核与配置差异](docs/freertos-changes.md) |
| 怎样回归验证，如何介绍设计选择 | [验证与面试](docs/validation-and-interview.md) |
| 裸机与 RTOS 的区别、历史来源 | [版本演进](docs/versions.md) |
...
撤销注入后，真实电压必须低于 4.10 V 并稳定 1 秒，才允许按恢复键清除。清除后产生另一条恢复记录。完整步骤和反例见[故障实验](docs/faults-and-cli.md)。
...
自编业务代码和文档采用 [MIT](LICENSE)。FreeRTOS 保留上游 MIT；ST/CMSIS 依赖按[安装教程](docs/getting-started.md)从用户持有的官方包恢复，未包含在公开源码中，详见[第三方说明](THIRD_PARTY_NOTICES.md)。
