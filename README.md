...
模块详解见[架构](docs/裸机架构.md)，历史协议及已知显示问题见[协议](docs/CAN通信协议.md)。该版本有演示性充放电状态与电压比例/显示缺陷；当前硬件没有充放电 MOS，故障状态只能报警、上报和记录，不能物理切断电流。后续 MOS 改造的裸机控制思路见[充放电 MOS 升级路线](docs/裸机架构.md#后续升级充放电-mos)，不能将该规划当作已经实现的功能，也不能与main的ECU-B混烧。
