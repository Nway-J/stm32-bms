 ...
 * data[4]低四位 = Alive Counter（0~15循环）；高四位固定为0；
...
#define CAN_ID_BMS_VOLTAGE    0x301  //电压
#define CAN_ID_BMS_TEMP       0x302  //温度
#define CAN_ID_BMS_FAULT      0x303  //故障
#define CAN_ID_CTRL_CMD       0x100  //控制命令ID
...
/*
 * 模块：监测运行状态的CAN编码扩展。
 * 用于0x301报文的data[3]，不是BMS_State_t枚举值。
 * 保留历史编码0、1、2，新增3表示监测运行。
 * ECU-B需要增加对应解码，旧版不能保证正确显示这个新值。
 */

// 系统正在监测，不表示电池正在充电或放电。
#define CAN_STATUS_MONITOR    3U

// Alive Counter占用低四位，因此最大值是15，下一次再回到0。
