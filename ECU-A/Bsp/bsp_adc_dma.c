...
// 诊断：记录DMA半传输中断进入次数。
volatile uint32_t g_dma_half_irq_count = 0U;

// 诊断：记录DMA全传输中断进入次数。
volatile uint32_t g_dma_full_irq_count = 0U;
...
void ADC_DMA_Init(void)
{
  ...
  DMA1_Channel1->CCR |= (1 << 10);    // MSIZE=16--存储数据宽度16位
	...
	/*
	 * 当前DMA中断只设置采样完成标志，不调用FreeRTOS API，
	 * 因此暂时使用高抢占优先级1。
	 */
	NVIC_SetPriority(
		DMA1_Channel1_IRQn,
